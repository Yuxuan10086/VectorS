...
bool Platform::load_chassis_params(chassis::DiffDriveParams & out)
{
  ...
  out.encoder_pulses_per_rev = encoder_pulses;
  ...
  if (out.encoder_pulses_per_rev <= 0.0) {
    RCLCPP_ERROR(node_.get_logger(), "encoder_pulses_per_rev 必须大于 0（当前配置 512000）");
    return false;
  }
  return true;
}
...
bool Platform::open()
{
  ...
  if (!chassis_->set_mode(chassis::DriveMode::kTwist)) {
    RCLCPP_ERROR(node_.get_logger(), "底盘 set_mode(kTwist) 失败");
    chassis_.reset();
    can_.close();
    return false;
  }
